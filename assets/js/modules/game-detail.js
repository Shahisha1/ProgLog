import { api, rawgToGame } from "./api.js";
import { getGame, saveGame, saveSession, addActivity } from "./store.js";
import { $, esc, fmtHours, showToast } from "./core.js";
function setupStars(value = 0) {
  const box = $("#starRating");
  if (!box) return;
  const current = Number(value) || 0;
  box.innerHTML = [1, 2, 3, 4, 5]
    .map(
      (n) =>
        `<button type="button" class="star ${n <= current ? "is-active" : ""}" data-rating="${n}" aria-label="${n} star${n > 1 ? "s" : ""}"><i data-lucide="star"></i></button>`,
    )
    .join("");
  box.querySelectorAll(".star").forEach((b) => {
    const preview = (r) =>
      box
        .querySelectorAll(".star")
        .forEach((x) =>
          x.classList.toggle("is-hover", Number(x.dataset.rating) <= r),
        );
    b.addEventListener("mouseenter", () => preview(Number(b.dataset.rating)));
    b.addEventListener("mouseleave", () =>
      box
        .querySelectorAll(".star")
        .forEach((x) => x.classList.remove("is-hover")),
    );
    b.addEventListener("focus", () => preview(Number(b.dataset.rating)));
    b.addEventListener("blur", () =>
      box
        .querySelectorAll(".star")
        .forEach((x) => x.classList.remove("is-hover")),
    );
    b.addEventListener("click", () => {
      const r = Number(b.dataset.rating);
      $("#personalRating").value = r;
      $("#ratingHint").textContent =
        `${r}/5 — ${["", "Not for me", "It was okay", "Worth playing", "Really good", "Loved it"][r]}`;
      box
        .querySelectorAll(".star")
        .forEach((x) =>
          x.classList.toggle("is-active", Number(x.dataset.rating) <= r),
        );
    });
  });
  window.lucide?.createIcons();
}
function portraitUrl(url) {
  if (!url) return "";
  return url.replace("/media/games/", "/media/crop/600/800/");
}
function imageCandidates(g) {
  const candidates = [];
  if (g.coverUrl) candidates.push(portraitUrl(g.coverUrl));
  if (g.coverUrl) candidates.push(g.coverUrl);
  if (g.heroUrl) candidates.push(g.heroUrl);
  for (const s of g.shortScreenshots || [])
    if (s.image) candidates.push(s.image);
  return [...new Set(candidates.filter(Boolean))];
}
function applyPortraitCover(g) {
  const cover = $("#gameCover");
  if (!cover) return;
  const candidates = imageCandidates(g);
  let i = 0;
  const next = () => {
    if (i >= candidates.length) return;
    const src = candidates[i++];
    cover.src = src;
  };
  cover.onerror = next;
  next();
}
export async function initGame() {
  const p = new URLSearchParams(location.search);
  let id = p.get("id"),
    rawgId = p.get("rawgId"),
    name = p.get("name") || "",
    game = null;
  if (id) game = await getGame(id);
  if (!game && rawgId)
    try {
      const d = await api(`/games/${rawgId}`);
      game = rawgToGame(d);
      game.id = `rawg-${game.rawgId}`;
    } catch {}
  if (!game && name)
    try {
      const r = await api(
        `/games?search=${encodeURIComponent(name)}&page_size=1`,
      );
      if (r.results?.[0]) {
        const d = await api(`/games/${r.results[0].id}`);
        game = rawgToGame(d);
        game.id = `rawg-${game.rawgId}`;
      }
    } catch {}
  if (!game) {
    $("#gamePage").innerHTML =
      '<div class="empty">This game could not be loaded.</div>';
    return;
  }
  try {
    const d = await api(`/games/${game.rawgId}`);
    game = { ...game, ...rawgToGame(d), id: game.id || `rawg-${game.rawgId}` };
    try {
      const a = await api(`/games/${game.rawgId}/achievements?page_size=100`);
      game.achievements = (a.results || []).map((x) => ({
        id: x.id,
        name: x.name,
        description: x.description || "",
        image: x.image || "",
        unlocked: Boolean(x.unlocked),
      }));
    } catch {}
    await saveGame(game);
    render(game);
  } catch {
    render(game);
  }
  $("#saveStatus")?.addEventListener("change", async (e) => {
    game.status = e.target.value;
    await saveGame(game);
    await addActivity("status_change", `${game.name} moved to ${game.status}`, {
      gameId: game.id,
    });
    showToast("Game status updated.");
  });
  $("#favoriteGame")?.addEventListener("click", async () => {
    game.favorite = !game.favorite;
    await saveGame(game);
    $("#favoriteGame").innerHTML = game.favorite
      ? '<i data-lucide="heart"></i> Favorited'
      : '<i data-lucide="heart"></i> Favorite';
    window.lucide?.createIcons();
    showToast(
      game.favorite ? "Added to favorites." : "Removed from favorites.",
    );
  });
  $("#reviewForm")?.addEventListener("submit", async (e) => {
    e.preventDefault();
    game.personalRating = Number($("#personalRating").value) || null;
    game.review = $("#personalReview").value.trim();
    await saveGame(game);
    await addActivity("review", `Reviewed ${game.name}`, { gameId: game.id });
    showToast("Your review was saved.");
  });
  $("#sessionForm")?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const minutes = Math.max(1, Number($("#sessionDuration").value) || 1);
    await saveSession({
      gameId: game.id,
      gameName: game.name,
      date: $("#sessionDate").value || new Date().toISOString().slice(0, 16),
      durationMinutes: minutes,
      note: $("#sessionNote").value.trim(),
    });
    game.playtimeHours = (Number(game.playtimeHours) || 0) + minutes / 60;
    await saveGame(game);
    showToast("Session logged.");
  });
}
function render(g) {
  document.title = `${g.name} | progLog`;
  $("#gameName").textContent = g.name;
  $("#gameMeta").textContent = `${
    (g.platforms || [])
      .slice(0, 4)
      .map((x) => x.platform?.name || x.name || x)
      .join(" · ") || "Game"
  }${g.released ? ` · ${g.released}` : ""}`;
  $("#gameRating").innerHTML =
    `<strong>${g.rating ? Number(g.rating).toFixed(1) : "—"}</strong> <span class="positive">RAWG</span>${g.metacritic ? ` · Metacritic ${g.metacritic}` : ""}`;
  applyPortraitCover(g);
  const art = $("#gameHero");
  if (art) {
    const hero = g.heroUrl || g.coverUrl;
    if (hero) {
      art.style.backgroundImage = `url("${hero}")`;
      art.classList.add("has-art");
      art.onerror = () => {
        if (g.coverUrl && hero !== g.coverUrl)
          art.style.backgroundImage = `url("${g.coverUrl}")`;
      };
    }
  }
  $("#gameDescription").innerHTML = esc(
    g.description || "No description available.",
  ).replace(/\n/g, "<br>");
  $("#saveStatus").value = g.status || "backlog";
  $("#favoriteGame").innerHTML = g.favorite
    ? '<i data-lucide="heart"></i> Favorited'
    : '<i data-lucide="heart"></i> Favorite';
  $("#personalRating").value = g.personalRating || "";
  $("#personalReview").value = g.review || "";
  setupStars(g.personalRating || 0);
  $("#ratingHint").textContent = g.personalRating
    ? `${g.personalRating}/5`
    : "Choose a rating from 1 to 5";
  $("#playtime").textContent = fmtHours(g.playtimeHours || 0);
  const ach = g.achievements || [];
  $("#achievementCount").textContent =
    `${ach.filter((a) => a.unlocked).length}/${ach.length}`;
  $("#gameReleased").textContent = g.released || "—";
  $("#gamePlatforms").textContent =
    (g.platforms || [])
      .slice(0, 2)
      .map((x) => x.platform?.name || x.name || x)
      .join(", ") || "—";
  const pct = ach.length
    ? Math.round((ach.filter((a) => a.unlocked).length / ach.length) * 100)
    : 0;
  $("#progressPercent").textContent = `${pct}%`;
  $("#progressHours").textContent = fmtHours(g.playtimeHours || 0);
  $("#progressAchievements").textContent =
    `${ach.filter((a) => a.unlocked).length}/${ach.length}`;
  const tags = [...(g.genres || []).slice(0, 4)];
  $("#gameTags").innerHTML = tags
    .map((t) => `<span class="game-tag">${esc(t.name || t)}</span>`)
    .join("");
  $("#gameAchievements").innerHTML = ach.length
    ? ach
        .slice(0, 12)
        .map(
          (a) =>
            `<div class="achievement ${a.unlocked ? "" : "locked"}"><img src="${esc(a.image || "../assets/images/achievement-placeholder.svg")}" alt="" loading="lazy"><div><strong>${esc(a.name || "Achievement")}</strong><small>${esc(a.description || "")}</small></div></div>`,
        )
        .join("")
    : '<div class="api-state">No RAWG achievement data for this game.</div>';
  const shots = (g.shortScreenshots || []).filter((x) => x.image);
  $("#gameScreenshots").innerHTML = shots.length
    ? shots
        .slice(0, 6)
        .map(
          (s) =>
            `<img src="${esc(s.image)}" alt="${esc(g.name)} gameplay screenshot" loading="lazy">`,
        )
        .join("")
    : '<div class="api-state">No screenshots available.</div>';
  window.lucide?.createIcons();
}
