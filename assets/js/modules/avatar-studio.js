...
export function setupAvatarStudio(initial = {}, editable = true) {
  ...
  const selectedCharacter = () =>
    GAME_CHARACTERS.find((x) => x.id === state.character);
  ...
  const persist = async () => {
    ...
    if (editable)
      await saveUserDoc({
        avatar: state,
        avatarPreset: state.preset || "character",
      });
    document
      .querySelectorAll("[data-user-avatar]")
      .forEach((x) => (x.src = avatarData(state)));
  };
  ...
  const openPicker = () => {
    ...
    modal
      .querySelector("#cancelAvatarPicker")
      ?.addEventListener("click", close);
    modal.addEventListener("click", (e) => {
      if (e.target === modal) close();
    });
    modal.querySelectorAll("[data-scroll]").forEach((button) =>
    ...
    modal.querySelectorAll("[data-character]").forEach((b) =>
    ...
    modal
      .querySelector("#confirmAvatarPicker")
