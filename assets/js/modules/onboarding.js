...
export async function maybeOnboard(user) {
  if (
    ...
    !db ||
    !user.uid ||
    !location.pathname.endsWith("overview.html")
  )
    return;
  addStyles();
  let done = false;
  try {
    done = localStorage.getItem(`proglog-onboarding-${user.uid}`) === "1";
  } catch (e) {}
  if (done) return;
  ...
  const openPicker = () => {
    ...
    picker
      .querySelector("#closeOnboardPicker")
      ?.addEventListener("click", close);
    picker.addEventListener("click", (e) => {
      if (e.target === picker) close();
    });
    picker.querySelectorAll("[data-scroll]").forEach((button) =>
      button.addEventListener("click", () => {
        const track = button
          .closest(".netflix-avatar-row")
          ?.querySelector(".netflix-character-track");
        track?.scrollBy({
          left: button.dataset.scroll === "next" ? 430 : -430,
          behavior: "smooth",
        });
      }),
    );
    picker.querySelectorAll("[data-character]").forEach((b) =>
      b.addEventListener("click", () => {
        ...
        selected = {
          ...
          character: c.id,
          game: c.game,
          imageUrl: GAME_CHARACTER_IMAGES[c.id],
          preset: "",
        };
        picker
          .querySelectorAll(".netflix-character")
          .forEach((x) => x.classList.remove("selected"));
        b.classList.add("selected");
      }),
    );
    picker.querySelectorAll("[data-preset]").forEach((b) =>
      b.addEventListener("click", () => {
    ...
    picker.querySelector("#useOnboardAvatar")?.addEventListener("click", () => {
      ...
      $("#onboardAvatarName").textContent =
        c?.name ||
        AVATAR_PRESETS.find((x) => x.id === selected.preset)?.name ||
        "Custom avatar";
      $("#onboardAvatarGame").textContent =
        c?.game || "progLog original pixel avatar";
      close();
    });
  };

  modal
    .querySelector("#openOnboardAvatar")
  ...
  modal
    ...
    .addEventListener("click", async () => {
      const btn = modal.querySelector("#finishOnboarding");
      btn.disabled = true;
      btn.innerHTML = "Saving…";
      try {
        const displayName =
          modal.querySelector("#onboardName").value.trim() || "Gamer";
        await setDoc(
        ...
        try {
          localStorage.setItem(`proglog-onboarding-${user.uid}`, "1");
          localStorage.setItem("proglog-avatar", JSON.stringify(selected));
        } catch (e) {}
        close();
