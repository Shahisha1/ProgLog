...
(function(){
  ...
  function resultHref(item){return item.kind==='vault' ? (window.pgRoute?window.pgRoute('game',{id:item.id}):'../../pages/game/game.html#'+encodeURIComponent(item.id)) : (window.pgRoute?window.pgRoute('game',{id:item.id}):'../../pages/game/game.html#'+encodeURIComponent(item.id))}
  function installSearch(){
    ...
    function render(q){
      ...
      var profs=safe(function(){return window.getProfiles?window.getProfiles():[]},[]);profs.filter(function(x){return !q||String(x.username||'').toLowerCase().includes(q)}).slice(0,4).forEach(function(x){rows.push('<a class="pg-command-row" href="'+esc(window.pgRoute?window.pgRoute('profile'):'../../pages/profile/profile.html')+'"><span class="pg-command-icon">◎</span><span><strong>'+esc(x.username||'Hunter')+'</strong><small>Profile</small></span><span class="pg-command-arrow">↗</span></a>')});var c=cabinet();var trophies=[];(c.games||[]).forEach(function(g){(g.achievements||[]).forEach(function(a){if(!q||String(a.name||'').toLowerCase().includes(q)||String(g.title||'').toLowerCase().includes(q))trophies.push({a:a,g:g})})});
